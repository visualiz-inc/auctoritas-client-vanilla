import { CacheLocation, AuctoritasClientOptions } from '../src/global';
...
import createAuth0Client, { AuctoritasClient } from '../src/index';
...
const setup = async (
  clientOptions: Partial<AuctoritasClientOptions> = {},
  ...
  const auctoritas = callConstructor
...
describe('Auctoritas', () => {
  ...
  describe('createAuth0Client()', () => {
    it('should create anAuctoritas client', async () => {
      const auctoritas = await createAuth0Client({
        domain: TEST_DOMAIN,
        client_id: TEST_CLIENT_ID
      });

      expect(auctoritas).toBeInstanceOf(AuctoritasClient);
    });
  ...
  describe('default creation function', () => {
    it('does nothing if there is nothing in storage', async () => {
      ...
      jest.spyOn(AuctoritasClient.prototype, 'getTokenSilently');
      ...
      expect(auctoritas.getTokenSilently).not.toHaveBeenCalled();
    });

    it('calls getTokenSilently if the authentication hint cookie is available`', async () => {
      AuctoritasClient.prototype.getTokenSilently = jest.fn();
      ...
      expect(auctoritas.getTokenSilently).toHaveBeenCalledWith(undefined);
    });
    ...
    describe('when refresh tokens are used', () => {
      it('creates the client with the correct scopes', async () => {
        ...
        expect((<any>auctoritas).scope).toBe('the-scope offline_access');
