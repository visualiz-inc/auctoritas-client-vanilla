...
describe('AuctoritasClient', () => {
  ...
  describe('handleRedirectCallback', () => {
    it('should not attempt to log the user in with Object prototype properties as state', async () => {
      ...
      await expect(auctoritas.handleRedirectCallback()).rejects.toThrow(
        'Invalid state'
      );
    });
    ...
    it('should clear the transaction data when the /authorize call redirects with a code param', async () => {
      ...
      jest.spyOn(auctoritas['transactionManager'], 'remove');

      await loginWithRedirect(auctoritas);

      expect(auctoritas['transactionManager'].remove).toHaveBeenCalledWith();
    });
    ...
    it('should throw an error if there is no transaction', async () => {
      ...
      let error;
      try {
        await auctoritas.handleRedirectCallback('test?foo=bar');
      } catch (e) {
        error = e;
      }
      expect(error).toBeDefined();
    ...
    it('returns the transactions appState', async () => {
      ...
      const result = await loginWithRedirect(auctoritas, { appState });
  ...
  it('calls oauth/token without redirect uri if not set in transaction', async () => {
    ...
    const auctoritas = setup();
    delete auctoritas['options']['redirect_uri'];

    await auctoritas.loginWithRedirect();
    await auctoritas.handleRedirectCallback();
