...
export const setupFn = mockVerify => {
  return (config?: Partial<AuctoritasClientOptions>, claims?: Partial<IdToken>) => {
...
export const loginWithRedirectFn = (mockWindow, mockFetch) => {
  return async (
    ...
    return await auctoritas.handleRedirectCallback(customCallbackUrl);
  };
};
...
export const loginWithPopupFn = (mockWindow, mockFetch) => {
  return async (
    ...
    await auctoritas.loginWithPopup(options, config);
  };
};

export const checkSessionFn = mockFetch => {
  return async auctoritas => {
    mockFetch.mockResolvedValueOnce(
      fetchResponse(true, {
        id_token: TEST_ID_TOKEN,
        refresh_token: TEST_REFRESH_TOKEN,
...
export const getTokenSilentlyFn = (mockWindow, mockFetch) => {
  return async (
    ...
    return await auctoritas.getTokenSilently(options);
  };
};
...
export const getTokenWithPopupFn = (mockWindow, mockFetch) => {
  return async (
    ...
    return await auctoritas.getTokenWithPopup(options);
  };
};
