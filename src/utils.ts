...
export const runIframe = (
    ...
    return new Promise<AuthenticationResult>((res, rej) => {
        ...
        const host = new URL(eventOrigin).origin;
        iframeEventHandler = function (e: MessageEvent) {
            ...
            if (!e.data || e.data.type !== 'authorization_response' || !e.data.response) {
                rej(GenericError.fromPayload(e.data));
            }
            else {
            ...
            // in Chrome: https://github.com/auctoritas/auctoritas-spa-js/issues/240
...
export const runPopup = (config: PopupConfigOptions) => {
    return new Promise<AuthenticationResult>((resolve, reject) => {
        ...
        popupEventListener = function (e: MessageEvent) {
            ...
            if (!e.data || e.data.type !== 'authorization_response' || !e.data.response) {
                return reject(GenericError.fromPayload(e.data));
            }
...
export const validateCrypto = () => {
    if (!getCrypto()) {
        throw new Error(
            'For security reasons, `window.crypto` is required to run `auctoritas-spa-js`.'
        );
    }
    if (typeof getCryptoSubtle() === 'undefined') {
        throw new Error(`
      auctoritas-spa-js must run on a secure origin. See https://github.com/auctoritas/auctoritas-spa-js/blob/master/FAQ.md#why-do-i-get-auctoritas-spa-js-must-run-on-a-secure-origin for more information.
    `);
    }
};
