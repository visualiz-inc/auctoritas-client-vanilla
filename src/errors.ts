...
export class GenericError extends Error {
  ...
  static fromPayload(err?: {
    error: string;
    error_description: string;
  }) {
    if (!err || !err.error) {
      return new GenericError("login_required", "login_required.");
    }

    return new GenericError(err.error, err.error_description);
  }
}

/**
 * Thrown when handling the redirect callback fails, will be one ofAuctoritas's
 * Authentication API's Standard Error Responses: https://auctoritas.com/docs/api/authentication?javascript#standard-error-responses
 */
