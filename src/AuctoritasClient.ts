 ...
 * @ignore
 */
const GET_TOKEN_SILENTLY_LOCK_KEY = 'auctoritas.lock.getTokenSilently';

/**
 * @ignore
 */
const buildOrganizationHintCookieName = (clientId: string) =>
    `auctoritas.${clientId}.organization_hint`;

/**
 * @ignore
 */
const OLD_IS_AUTHENTICATED_COOKIE_NAME = 'auctoritas.is.authenticated';

/**
 * @ignore
 */
const buildIsAuthenticatedCookieName = (clientId: string) =>
    `auctoritas.${clientId}.is.authenticated`;
...
const getCustomInitialOptions = (
    options: AuctoritasClientOptions
): BaseLoginOptions => {
    const {
        advancedOptions,
        audience,
 ...
 */
export default class AuctoritasClient {
    ...
    constructor(private options: AuctoritasClientOptions) {
        typeof window !== 'undefined' && validateCrypto();

        if (options.cache && options.cacheLocation) {
            console.warn(
                'Both `cache` and `cacheLocation` options have been specified in theAuctoritasClient configuration; ignoring `cacheLocation` and using `cache`.'
            );
        }
    ...
    private _url(path: string) {
        const auctoritasClient = encodeURIComponent(
            btoa(JSON.stringify(this.options.auctoritasClient || DEFAULT_AUTH0_CLIENT))
        );
        return `${this.domainUrl}${path}&auctoritasClient=${auctoritasClient}`;
    }
    ...
    /**
     * ```js
     * await auctoritas.buildAuthorizeUrl(options);
     * ```
     *
     ...
     * ```js
     * try {
     *  await auctoritas.loginWithPopup(options);
     * } catch(e) {
        ...
        const authResult = await oauthToken(
            {
                ...
                redirect_uri: params.redirect_uri,
                auctoritasClient: this.options.auctoritasClient,
    ...
    /**
     * ```js
     * const claims = await auctoritas.getIdTokenClaims();
     * ```
     *
    ...
    /**
     * ```js
     * await auctoritas.loginWithRedirect(options);
     * ```
     *
     ...
     * responses fromAuctoritas. If the response is successful, results
     ...
     *
     * This method also heeds the `auctoritas.{clientId}.is.authenticated` cookie, as an optimization
     *  to prevent callingAuctoritas unnecessarily. If the cookie is not present because
     * there was no previous login (or it has expired) then tokens will not be refreshed.
     *
     * It should be used for silently logging in the user when you instantiate the
     * `AuctoritasClient` constructor. You should not need this if you are using the
    ...
    /**
     * ```js
     * const token = await auctoritas.getTokenSilently(options);
     * ```
     *
    ...
    /**
     * ```js
     * const token = await auctoritas.getTokenWithPopup(options);
     * ```
     * Opens a popup with the `/authorize` URL using the parameters
     * provided as arguments. Random and secure `state` and `nonce`
     * parameters will be auto-generated. If the response is successful,
    ...
    /**
     * ```js
     * const isAuthenticated = await auctoritas.isAuthenticated();
     * ```
     *
    ...
    /**
     * ```js
     * await auctoritas.buildLogoutUrl(options);
     * ```
     *
     ...
     * the parameters provided as arguments, to clear theAuctoritas session.
     *
     ...
     * [Read more about how Logout works atAuctoritas](https://auctoritas.com/docs/logout).
     *
     * @param options
     */
